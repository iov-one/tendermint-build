...
# This file was copied from the https://github.com/cosmos/gaia project with minor changes.
...
DEFAULT_GBUILD_FLAGS='--memory 4000'
DEFAULT_SIGS_REPO='https://github.com/iov-one/tendermint.sigs'
...
f_builddir() {
  printf '%s' "${g_workdir}/tendermint-build-$1"
}
...
f_prep_build() {
  ...
  for l_os in ${l_platforms}; do
    ...
    cp -a "${g_cached_gitian}" "${l_builddir}" >&2
    mkdir -p "${l_builddir}/inputs/"
...
f_commit_sig() {
  ...
  pushd "${g_sigs_dir}"
  git add -f ${l_release_name} || echo "git add failed" >&2
...
for g_os in ${g_platforms}; do
  g_release="${BUILD_TAG}"
  g_descriptor="${g_workdir}/contrib/gitian-descriptors/tendermint-${g_os}.yml"
  [ -f "${g_descriptor}" ]
